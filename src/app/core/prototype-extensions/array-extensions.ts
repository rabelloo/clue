...
function flatMap<T, U>(
  propertyFn: (item: T, index: number, array: T[]) => U[],
  thisArg?: any
): U[] {
  return this.reduce(
...
function groupBy<T>(
  propertyFn: (item: T, index: number, array: T[]) => number | string
): { [id: string]: T[] } {
  return this.reduce((group, item, index, array) => {
...
function toHashMap<T>(
  propertyFn: (item: T, index: number) => string | number
): { [id: string]: T } {
  return this.reduce(
    (hashMap, item, index) => ({
      ...hashMap,
